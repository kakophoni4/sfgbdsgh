...
RE_FAKE_FIRM_NAME = re.compile(
    ...
    r"с\s+(?:больш\w+\s+)?оборот|без\s+оборот|под\s+ключ|в\s+любой\s+регион|"
    r"профи\b|экспресс|срочно|недорого|дешев|"
    r"\d{4}\s*г|история\s+\d{4}|"
    # гео/объявленческий хвост, не бренд: «ООО в Москве», «ООО на УСН»
    r"в\s+москв|в\s+спб|в\s+питер|в\s+ростов|в\s+любом|"
    r"на\s+усн|на\s+осно|под\s+ключ|с\s+адресом|без\s+адрес|"
    r"регистрация\s+\d|дата\s+регистр|год\s+основан|основан[аи]\s+\d"
    r")"
)
...
def is_plausible_firm_name(name: str) -> bool:
    ...
    if re.search(
        r"(?i)историей|по\s+запросу|нулевк|нулёвк|больш\w*\s+оборот|с\s+оборот",
        core,
    ):
        return False
    return True
...
def can_search_egrul_by_name(name: str) -> bool:
    ...
    if not re.match(r"(?i)^ООО\b", s):
        return False
    # объявленческий мусор (не юр.имя)
    if re.search(
        r"(?i)"
        r"в\s+москв|москва\s*/?\s*мо|москва\s+или|адрес\s*:|регион\s*:|"
        r"налог|открыта\s+в|новая\s+компани|уставн\w*\s+капитал|"
        r"инн\s*\d|^\s*ООО\s*«?\s*ИНН|^\s*ООО\s*,|^\s*ООО\s*«?\s*\d+\.|"
        r"регистрация\s+\d|без\s+смен|бес\s+смен|с\s+р/?с|с\s+эцп|"
        r"с\s+минцифр|на\s+усн|на\s+осно|на\s+осн\b|на\s+чел|"
        r"возраст\s+от|чтобы\s+\d|люб\s*рег|выручка\s+от|"
        r"автомобил|эцп|переведен",
        s,
    ):
        return False
    ...
    if len(core) < 5:
        return False
    # одно короткое слово 1–5 букв (СИЛА/МЕГА/АВТО слишком общие)
    if re.fullmatch(r"[А-ЯЁA-Zа-яёa-z]{1,5}", core):
        return False
    if re.match(r"^\d", core):
        return False
    if re.match(r"(?i)^(в|на|под|без|бес|с|для|по|от)\s+", core):
        return False
    # много слов / целое предложение из объявления
    if len(core.split()) >= 4:
        return False
    if len(core) > 60:
        return False
